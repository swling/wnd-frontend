...
abstract class Wnd_User_Auth {
	...
	public static function update_user_openid(int $user_id, string $type, string $open_id): int {
		...
		$ID          = $auth_record->ID ?? 0;
		if ($ID) {
			$db = static::update_db($ID, $user_id, $type, $open_id);
		} else {
			$db = static::insert_db($user_id, $type, $open_id);
		}
	...
	public static function delete_user_openid(int $user_id, string $type): int{
		...
		$user    = static::get_user_auths($user_id);
		$open_id = $user->$type ?? '';
		$db      = static::delete_db($type, $open_id);
	 ...
	 * @since 0.9.57.1
	 */
	public static function update_auth_cache(int $user_id, string $type, string $open_id) {
	 ...
	 * @since 0.9.57.1
	 */
	public static function clean_auth_caches(int $user_id) {
		// 用户对象集合
	 ...
	 * @since 0.9.36
	 */
	public static function get_auth_records(int $user_id) {
		global $wpdb;
