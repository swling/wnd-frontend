...
class Wnd_Reg_Form extends Wnd_Module {
	...
	public static function build($type = 'email') {
		...
		if (!get_option('users_can_register')) {
			return self::build_error_message(__('站点已关闭注册', 'wnd'));
		}

		//未开启手机验证
		if ('phone' == $type and wnd_get_option('wnd', 'wnd_enable_sms') != 1) {
			return self::build_error_message(__('当前未配置短信验证', 'wnd'));
		}

		// 关闭了邮箱注册（强制手机验证）
		if ('email' == $type and 1 == wnd_get_option('wnd', 'wnd_disable_email_reg')) {
			return self::build_error_message(__('当前设置禁止邮箱注册', 'wnd'));
		}
		...
		if (wnd_get_option('wnd', 'wnd_agreement_url') or 1) {
			$text = __('已阅读并同意', 'wnd') . '<a href="' . wnd_get_option('wnd', 'wnd_agreement_url') . '" target="_blank">' . __('《注册协议》', 'wnd') . '</a>';
			$form->add_checkbox(
				[
					'name'     => 'agreement',
					'options'  => [$text => 1],
