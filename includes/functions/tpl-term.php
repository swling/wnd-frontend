 ...
 */
function wnd_term_select_query_arg() {
 ...
 *@since 2019.04.25
 */
function wnd_terms_checkbox($taxonomy, $value = 'slug', $name = '', $require = false) {
	if ($name == '') {
		$name = $taxonomy;
	}
 ...
 *###################################################### 表单设置：标签编辑器
 */
function wnd_tags_editor_script($maxTags = 3, $maxLength = 10, $placeholder = '标签', $taxonomy = '') {
	...
	jQuery(document).ready(function($) {
		$("#tags").tagEditor({
			...
			autocomplete: {
				...
				source: [' . wnd_terms_text($taxonomy, 200) . ']
			},
